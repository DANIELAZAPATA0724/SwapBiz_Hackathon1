...
const CategoriesModel = {
  ...
  getCategory: async (categoryId) => {
    const query = 'SELECT * FROM service_categories WHERE category_id = $1';
    const { rows } = await pool.query(query, [categoryId]);
    return rows[0];
  },
  createNewCategory: async (name) => {
    const query = 'INSERT INTO service_categories (name) VALUES ($1) RETURNING *';
    const { rows } = await pool.query(query, [name]);
    return rows[0];
  },
  updateOneCategory: async (categoryId, newName) => {
    const query = 'UPDATE service_categories SET name = $1 WHERE category_id = $2 RETURNING *';
